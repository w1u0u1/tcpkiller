...
namespace tcpkiller
{
    class Disconnecter
    {
        ...
        [DllImport("wsock32.dll")]
        private static extern ushort htons(ushort netshort);
        ...
        public static void CloseRemoteIP(string IP, int port)
        {
            MIB_TCPROW[] rows = getTcpTable();
            for (int i = 0; i < rows.Length; i++)
            {
                if (rows[i].dwRemoteAddr == IPStringToInt(IP) && ntohs(rows[i].dwRemotePort) == port)
                {
                    rows[i].dwState = (int)State.Delete_TCB;
                    IntPtr ptr = GetPtrToNewObject(rows[i]);
                    int ret = SetTcpEntry(ptr);
        ...
        public static void CloseConnection(string connectionstring)
        {
            try
            {
                ...
                row.dwLocalPort = htons(ushort.Parse(loc[1]));
                row.dwRemotePort = htons(ushort.Parse(rem[1]));
        ...
        public static string[] Connections(State state)
        {
            ...
            foreach (MIB_TCPROW row in rows)
            {
                if (state == State.All || state == (State)row.dwState)
                {
                    ...
                    arr.Add(string.Format("{0,-24}{1,-24}{2}", localaddress, remoteaddress, ((State)row.dwState).ToString()));
                }
            }
        ...
        private static MIB_TCPROW[] getTcpTable()
        {
            ...
            try
            {
                ...
                buffSubPointer = (IntPtr)((long)buffer + 4); //Move to the first data (ignoring dwNumEntries from the original MIB_TCPTABLE struct) 
                ...
                for (int i = 0; i < structCount; i++)
                {
                    ...
                    buffSubPointer = (IntPtr)((long)buffSubPointer + sizeOfTCPROW); //move to next structdata 
                }
